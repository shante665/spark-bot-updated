...
function Sparkbot(configuration) {
  ...
  if (!controller.config.public_address) {
    ...
    if (!endpoint.hostname) {
      ...
      throw new Error(
        "Please specify an SSL-enabled url for your public address: " +
          controller.config.public_address
      );
    } else {
  ...
  if (!controller.config.secret) {
    console.log(
      "WARNING: No secret specified. Source of incoming webhooks will not be validated. https://developer.ciscospark.com/webhooks-explained.html#auth"
    );
    ...
    var list = controller.api.webhooks.list().then(function(list) {
      ...
      if (hook_id) {
        controller.api.webhooks
          ...
          .then(function(res) {
            console.log(
              "Cisco Spark: SUCCESSFULLY UPDATED CISCO SPARK WEBHOOKS"
            );
            if (cb) cb();
          })
          .catch(function(err) {
        ...
        controller.api.webhooks
          ...
          .then(function(res) {
            console.log(
              "Cisco Spark: SUCCESSFULLY REGISTERED CISCO SPARK WEBHOOKS"
            );
            if (cb) cb();
          })
          .catch(function(err) {
  ...
  controller.on("message_received", function(bot, message) {
    if (controller.config.limit_to_org) {
      console.log(
        "limit to org",
        controller.config.limit_to_org,
        message.original_message.orgId
      );
      if (
        !message.original_message.orgId ||
        message.original_message.orgId != controller.config.limit_to_org
      ) {
  ...
  controller.handleWebhookPayload = function(req, res, bot) {
    if (controller.config.secret) {
      ...
      var hash = crypto
        .createHmac("sha1", controller.config.secret)
        ...
        .then(function(message) {
          ...
          if (message.original_message.html) {
            ...
            var pattern = new RegExp(
              '^\<p\>\<spark\-mention .*?data\-object\-id\="' +
            ...
            var pattern = new RegExp(
              "^" + controller.identity.displayName + "\\s+",
              "i"
            );
            if (message.text) {
  ...
  controller.defineBot(function(botkit, config) {
    ...
    bot.startPrivateConversationWithActor = function(message, cb) {
      bot.startPrivateConversationWithPersonId(
        message.original_message.actorId,
        cb
      );
    };
        ...
        function(err, response, body) {
          if (!err) {
            ...
            if (obj["content-disposition"]) {
              obj.filename = obj["content-disposition"].replace(
                /.*filename=\"(.*)\".*/gi,
                "$1"
              );
            }
